...
//Why is register pink? Stupid reserved words.
...
-(NSDictionary*)taskStop:(NSString*)taskID{
    NSArray* array = [NSArray arrayWithObjects:@"pro.task_stop",_session.token,taskID,nil];//task_status?
    return [_session execute:array];
}
...
-(NSDictionary*)getCwd:(NSString*)sessionID path:(NSString*)path{
    NSArray* array = [NSArray arrayWithObjects:@"pro.meterpreter_getcwd",_session.token,sessionID,path,nil];
    return [_session execute:array];
}
