...
-(NSDictionary*)ringPut:(NSString*)sessionID data:(NSString*)data{
    NSArray* array = [NSArray arrayWithObjects:@"session.ring_put",_session.token,sessionID,data,nil];//Same as session.shell_write
    return [_session execute:array];
}
-(NSDictionary*)ringRead:(NSString*)sessionID{
    NSArray* array = [NSArray arrayWithObjects:@"session.ring_read",_session.token,sessionID,nil];//Same as session.shell_read
    return [_session execute:array];
}
-(NSDictionary*)ringRead:(NSString *)sessionID withReadPointer:(NSString*)readPointer{
    NSArray* array = [NSArray arrayWithObjects:@"session.ring_read",_session.token,sessionID,readPointer,nil];//Same as session.shell_read
    return [_session execute:array];
}
